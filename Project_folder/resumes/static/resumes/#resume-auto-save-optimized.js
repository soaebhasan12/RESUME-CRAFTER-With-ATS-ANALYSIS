...
function showSavingIndicator() {
    document.querySelector('.saving-indicator')?.classList.add('show');
}

function hideSavingIndicator() {
    document.querySelector('.saving-indicator')?.classList.remove('show');
}
    ...
    function setupFormListeners() {
        ...
        formElements.forEach(element => {
            if (element.type === 'file') {
                element.addEventListener('change', triggerSave);
            } else {
                element.addEventListener('input', triggerSave);
                element.addEventListener('change', triggerSave);
            }
        });
            ...
            mutations.forEach(mutation => {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) { // Element node
                        const newElements = node.querySelectorAll('input, textarea, select');
